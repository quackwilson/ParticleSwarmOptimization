    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 201; i++)
            {
                ...
                using (var writer = new StreamWriter("C:/Users/quack/Desktop/ShubertFunction.csv", append))
        ...
        static double ShubertFunction(double x0, double x1)
        {
            ...
            for (int i = 1; i <= 5; i++)
            {
                a = a + (i * Math.Cos(((i + 1) * x0) + i));
                b = b + (i * Math.Cos(((i + 1) * x1) + i));
            }
            double ans = a * b;
            return ans;
        }
    ...
    class Particle
    {
        public Particle(Random num, int variableNumber, int particleNumber)
        {
            ...
            for (int i = 0; i < variableNumber; i++)
            {
                ...
                {
                    double ran = NextDouble(num, -10.00, 10.00);
                    particles[i, j] = ran;
                    Particles = particles;
                }
            }
